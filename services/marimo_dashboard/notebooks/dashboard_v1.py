...
def _(mo, api_base_url, requests):
    ...
    mem_defaults = ["memory.percent_usage", "memory.used"]

    cpu_selected_default = [m for m in cpu_defaults if m in available_metrics]
    other_selected_default = [m for m in mem_defaults if m in available_metrics]
    ...
    manual_refresh = mo.ui.button(label="Обновить данные", kind="success")
...
@app.cell
def _(api_base_url, requests, pd, cpu_metrics_selector, other_metrics_selector, time_window, refresh_interval, auto_refresh, manual_refresh):
    ...
    return fetch_series, selected_cpu_metrics, selected_other_metrics, selected_metrics, minutes, interval, series_data, stats_data
...
@app.cell
def _(mo, manual_refresh, refresh_interval):
    mo.md(
        f"""
### Управление

- Ручное обновление: нажми кнопку **Обновить данные**.
